...
static void win32_ser_init(struct win32_ser *ws) {
	...
	ws->ol.hEvent = INVALID_HANDLE_VALUE;
}
...
static int win32_ser_select(struct win32_ser *ws, int max_len, struct timeval *tv) {
    ...
    if (ReadFile(ws->fd, &ws->buf, max_len, &ws->n_bytes, &ws->ol)) {
		...
		else if (WAIT_OBJECT_0 == WaitForSingleObject(ws->ol.hEvent, 1000))
		{
			if (!GetOverlappedResult(ws->fd, &ws->ol, &ws->n_bytes, FALSE))
				return -1;
...
static int _modbus_rtu_connect(modbus_t *ctx)
{
	...
	ctx_rtu->w_ser.ol.hEvent = CreateEvent(NULL, TRUE, FALSE, NULL);
    /* Error checking */
	if (ctx_rtu->w_ser.ol.hEvent == NULL) {
        fprintf(stderr, "ERROR Can't create overlapped event \n",
                strerror(errno));
...
void _modbus_rtu_close(modbus_t *ctx)
{
	...
	CloseHandle(ctx_rtu->w_ser.ol.hEvent);
#else
